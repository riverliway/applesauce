...
const WEBSOCKET_URL = localStageProd({
  local: 'ws://apple-api.riverway.li:4000',
  prod: 'ws://apple-api.riverway.li:4000'
})
...
export interface WebsocketContext {
   ...
   */
  startSimulation: (width: number, height: number, numBots: number) => void
  /**
...
export const WebsocketProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const value = {
    register,
    unregister,
    startSimulation: (width: number, height: number, numBots: number) => {
      sendWsMessage({
